...
import {
  ...
  ChevronLeft, ChevronRight, Package, Clock, ImageIcon
} from 'lucide-react'
...
const [searchTelechargements, setSearchTelechargements] = useState('')
    ...
    const fetchRessources = async () => {
        try { const res = await api.get('/resources'); setRessources(res.data) }
        catch (err) { console.error(err) }
    }
    const fetchMesTelechargements = async () => {
        try { const res = await api.get('/users/mes-telechargements'); setMesTelechargements(res.data) }
        catch (err) { console.error(err) }
    }
    ...
    const fetchRessourcesPhysiques = async () => {
        try { const res = await api.get('/ressources-physiques'); setRessourcesPhysiques(res.data) }
        catch (err) { console.error(err) }
    }
    const fetchMesDemandes = async () => {
        try { const res = await api.get('/demandes/mes-demandes'); setMesDemandes(res.data) }
        catch (err) { console.error(err) }
    }
    ...
    const RessourceCard = ({ r }) => (
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-5 flex flex-col justify-between h-full min-h-[260px]">
            ...
            <div className="flex gap-2">
                ...
                <button onClick={() => {
                    if (!r.fichier) { alert('Aucun fichier disponible'); return }
                    window.open(`http://localhost:5000/uploads/${r.fichier}`, '_blank')
            ...
            items.length > 0 && (
                <div className="mb-10 group relative">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button onClick={() => setSelectedCategory(title)} className="text-base text-indigo-600 hover:underline transition">Voir plus</button>
                    </div>
                    ...
                    <div ref={sliderRef} className="flex gap-5 overflow-x-auto scroll-smooth snap-x snap-mandatory scrollbar-hide items-stretch">
                        {items.map(r => (
                            <div key={r._id} className="min-w-[280px] max-w-[280px] flex-shrink-0 snap-start">
                                <RessourceCard r={r}/>
                            </div>
                        ))}
                    </div>
                    {selectedCategory && (
                        <div className="fixed inset-0 bg-slate-50 z-50 overflow-y-auto">
                            <div className="sticky top-0 z-20 bg-white border-b border-slate-100 px-8 py-4 flex justify-between items-center shadow-sm">
                                ...
                                <button onClick={() => setSelectedCategory(null)} className="w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center text-slate-500 hover:bg-slate-200 transition font-bold text-lg">✕</button>
                            </div>
                            <div className="p-8 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                                {ressources
                                    .filter(r => selectedCategory === 'Nouveautés' ? true : r.discipline?.toLowerCase().includes(selectedCategory.toLowerCase()))
        ...
        return (
            items.length > 0 && (
                <div className="mb-10 group relative">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-xl font-bold text-slate-800">{title}</h2>
                        <button onClick={() => setSelectedCategoryPhysique(title)} className="text-base text-indigo-600 hover:underline transition">Voir plus</button>
                    </div>
                    <button onClick={scrollLeft} className="absolute left-0 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-black/40 backdrop-blur flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                        <ChevronLeft className="text-white" size={26}/>
                    </button>
                    ...
                    {selectedCategoryPhysique && (
                        <div className="fixed inset-0 bg-slate-50 z-50 overflow-y-auto">
                            <div className="sticky top-0 z-20 bg-white border-b border-slate-100 px-8 py-4 flex justify-between items-center shadow-sm">
                                ...
                                <button onClick={() => setSelectedCategoryPhysique(null)} className="w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center text-slate-500 hover:bg-slate-200 transition font-bold text-lg">✕</button>
                            </div>
                            <div className="p-8 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
                                {ressourcesPhysiques
                                    .filter(r => r.discipline?.toLowerCase().includes(selectedCategoryPhysique.toLowerCase()))
            ...
            <aside className="w-64 bg-slate-900 flex flex-col border-r border-slate-800">
                ...
                <nav className="flex-1 px-4 pt-4 space-y-2">
                    {[
                        ...
                        { id: 'emprunts', label: 'Ressources physiques', icon: <Package size={18}/> },
                        { id: 'demandes', label: 'Mes demandes', icon: <Clock size={18}/> },
                ...
                <div className="p-4 border-t border-slate-800">
                    <div className="flex items-center gap-3 mb-3 px-2">
                        ...
                        <div>
                            ...
                            <p className="text-indigo-300 text-xs">{user?.prenom} {user?.nom}</p>
                        </div>
                    </div>
                    <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2 rounded-xl text-red-300 hover:bg-red-500/10 transition text-sm font-medium">
                ...
                <header className="relative z-50 h-16 bg-white/80 backdrop-blur border-b border-indigo-100 flex items-center justify-between px-8 shadow-sm">
                    <div className="relative">
                        <Search className="absolute left-3 top-2.5 text-slate-400" size={16}/>
                            <input 
                                value={
                                    activeTab === 'emprunts' ? searchPhysique :
                                    activeTab === 'demandes' ? searchDemandes :
                                    activeTab === 'telechargements' ? searchTelechargements :
                                    search
                                }
                                onChange={e => {
                                    if (activeTab === 'emprunts') setSearchPhysique(e.target.value)
                                    else if (activeTab === 'demandes') setSearchDemandes(e.target.value)
                                    else if (activeTab === 'telechargements') setSearchTelechargements(e.target.value)
                                    else setSearch(e.target.value)
                                }}
                                placeholder={
                                    activeTab === 'emprunts' ? "Rechercher un livre physique..." :
                                    activeTab === 'demandes' ? "Rechercher une demande..." :
                                    activeTab === 'telechargements' ? "Rechercher un téléchargement..." :
                                    "Rechercher..."
                                }
    ...
    <div>
        {searchPhysique ? (
            <div>
                <h2 className="text-xl font-bold text-slate-800 mb-4">Résultats</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
                    {ressourcesPhysiques
                        .filter(r => r.titre?.toLowerCase().includes(searchPhysique.toLowerCase()) ||
            ...
            <table className="w-full">
                <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                        {['Livre', 'Auteur', 'Date demande', 'Statut', 'Action'].map(h => (
                ...
                <tbody>
                    {mesDemandes.length === 0 ? (
                        ...
                        .filter(d => d.ressource?.titre?.toLowerCase().includes(searchDemandes.toLowerCase()) ||
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        {mesTelechargements.length === 0 ? (
                                            ...
                                            .filter(r => r.titre?.toLowerCase().includes(searchTelechargements.toLowerCase()))
                                            .map(r => (
                                            <tr key={r._id} className="border-b border-slate-50 hover:bg-slate-50 transition">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                                                        r.type === 'livre' ? 'bg-orange-100 text-orange-700' :
                                                        r.type === 'support cours' ? 'bg-blue-100 text-blue-700' :
                        ...
                        <div className="max-w-5xl mx-auto">
                            ...
                            <div className="bg-white rounded-[32px] overflow-hidden border border-slate-200 shadow-xl">
                                <div className="h-32 bg-gradient-to-r from-slate-800 to-slate-900 relative">
                                    <button onClick={() => {
                                        setEditData({ nom: user?.nom || '', prenom: user?.prenom || '', email: user?.email || '', filiere: user?.filiere || '' })
                                ...
                                <div className="px-10 pb-10 relative">
                                    ...
                                    <div className="pt-20">
                                        ...
                                        <span className="inline-block mt-3 px-4 py-1.5 bg-indigo-100 text-indigo-700 rounded-full text-sm font-bold">Étudiant</span>
                                    </div>
                                ...
                                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
                                    <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md">
                                        ...
                                        <div className="flex gap-3 mt-8">
                                            ...
                                            }}  className="flex-1 h-12 rounded-2xl bg-slate-700 text-white font-bold hover:opacity-90 transition">
                                                Enregistrer
                                            </button>
                                        </div>
                                    </div>
